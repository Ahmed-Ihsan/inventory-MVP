  ...
  (
    ...
    const renderInput = () => {
      const commonProps = {
        ...
        placeholder,
        'aria-describedby':
          [help && !hasError ? `${fieldId}-help` : null, hasError ? `${fieldId}-error` : null]
            .filter(Boolean)
            .join(' ') || undefined,
      ...
      if (type === 'textarea') {
        return (
          <div className="input-wrapper">
            <textarea {...commonProps} rows={rows} className={`input ${fieldClass}`} />
            {showCount && maxLength && (
              <span
                className={`char-counter${charCount >= maxLength ? ' char-counter-limit' : ''}`}
              >
                {charCount}/{maxLength}
              </span>
            )}
            ...
            {options.map((option) => (
              <div key={option.value} className="radio-item">
                <input
                  ...
                  style={{ width: 'auto', marginRight: '8px' }}
                />
                <label style={{ margin: 0, fontWeight: 'normal' }}>{option.label}</label>
              </div>
            ))}
          </div>
        );
      ...
      return (
        <div
          className={`input-wrapper${hasPrefix ? ' has-prefix' : ''}${hasSuffix ? ' has-suffix' : ''}`}
        >
          ...
          {hasSuffix && (
            <span className="input-suffix-actions">
              ...
              {suffix && !isPassword && <span className="input-suffix-icon">{suffix}</span>}
              {isPassword && (
                <button
                  type="button"
                  className="input-action-btn"
